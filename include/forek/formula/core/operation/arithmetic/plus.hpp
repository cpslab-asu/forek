#ifndef FOREK_FORMULA_CORE_OPERATION_ARITHMETIC_PLUS_HPP
#define FOREK_FORMULA_CORE_OPERATION_ARITHMETIC_PLUS_HPP
...
template <typename T>
class Plus : public Binary<T> {
   public:
    Plus() = delete;
    Plus(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Plus with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Plus>(std::move(this->lexpr_->clone()),
