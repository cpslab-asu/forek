#ifndef FOREK_FORMULA_CORE_OPERATION_ARITHMETIC_MINUS_HPP
#define FOREK_FORMULA_CORE_OPERATION_ARITHMETIC_MINUS_HPP
...
template <typename T>
class Minus : public Binary<T> {
   public:
    Minus() = delete;
    Minus(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
