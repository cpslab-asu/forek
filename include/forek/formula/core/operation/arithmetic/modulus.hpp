#ifndef FOREK_FORMULA_CORE_OPERATION_ARITHMETIC_MODULUS_HPP
#define FOREK_FORMULA_CORE_OPERATION_ARITHMETIC_MODULUS_HPP
...
template <typename T>
class Modulus : public Binary<T> {
   public:
    Modulus() = delete;
    Modulus(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Modulus with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Modulus>(std::move(this->lexpr_->clone()),
