#ifndef FOREK_FORMULA_CORE_OPERATION_MTL_EVENTUALLY_HPP
#define FOREK_FORMULA_CORE_OPERATION_MTL_EVENTUALLY_HPP
...
template <typename T>
class EventuallyBounded : public Unary<T> {
   public:
    const types::Interval interval_;

   public:
    EventuallyBounded() = delete;
    EventuallyBounded(std::unique_ptr<core::Node<T>> expr, types::Interval interval)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException(
                "unable to visit EventuallyBounded with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<EventuallyBounded>(std::move(this->expr_->clone()),
                                                   this->interval_);
    }
};
}  // namespace forek::formula::core::operation::mtl
