#ifndef FOREK_FORMULA_CORE_OPERATION_MTL_UNTIL_HPP
#define FOREK_FORMULA_CORE_OPERATION_MTL_UNTIL_HPP
...
template <typename T>
class UntilBounded : public Binary<T> {
   public:
    const types::Interval interval_;

   public:
    UntilBounded() = delete;
    UntilBounded(std::unique_ptr<core::Node<T>> lexpr, std::unique_ptr<core::Node<T>> rexpr,
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit UntilBounded with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<UntilBounded>(std::move(this->lexpr_->clone()),
