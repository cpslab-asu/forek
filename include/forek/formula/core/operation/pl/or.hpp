#ifndef FOREK_FORMULA_CORE_OPERATION_PL_OR_HPP
#define FOREK_FORMULA_CORE_OPERATION_PL_OR_HPP
...
template <typename T>
class Or : public Binary<T> {
   public:
    Or() = delete;
    Or(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Or with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Or>(std::move(this->lexpr_->clone()),
