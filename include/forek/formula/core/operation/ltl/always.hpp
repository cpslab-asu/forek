...
class Always : public Unary {
   public:
    Always() = delete;
    explicit Always(std::unique_ptr<Node> expr) : Unary(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor& visitor) const -> void override {
        try {
            ...
            throw visitor::VisitorException(
                "unable to visit Always with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const
        -> std::unique_ptr<core::Node> override {
        return std::make_unique<Always>(std::move(expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::ltl
#endif
