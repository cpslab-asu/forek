...
template <typename T>
class Eventually : public Unary<T> {
   public:
    Eventually() = delete;
    explicit Eventually(std::unique_ptr<Node<T>> expr) : Unary<T>(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Eventually with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Eventually>(std::move(this->expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::ltl
#endif
