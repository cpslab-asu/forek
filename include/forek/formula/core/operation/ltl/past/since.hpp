#ifndef FOREK_FORMULA_CORE_OPERATION_LTL_PAST_SINCE_HPP
#define FOREK_FORMULA_CORE_OPERATION_LTL_PAST_SINCE_HPP
...
template <typename T>
class Since : public Binary<T> {
   public:
    Since() = delete;
    Since(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Since with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Since>(std::move(this->lexpr_->clone()),
