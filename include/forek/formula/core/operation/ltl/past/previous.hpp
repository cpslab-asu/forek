#ifndef FOREK_FORMULA_CORE_OPERATION_LTL_PAST_PREVIOUS_HPP
#define FOREK_FORMULA_CORE_OPERATION_LTL_PAST_PREVIOUS_HPP
...
template <typename T>
class Previous : public Unary<T> {
   public:
    Previous() = delete;
    explicit Previous(std::unique_ptr<Node<T>> expr) : Unary<T>(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Previous with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Previous>(std::move(this->expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::past::ltl
#endif
