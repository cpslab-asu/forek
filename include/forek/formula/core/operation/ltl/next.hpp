#ifndef FOREK_FORMULA_CORE_OPERATION_LTL_NEXT_HPP
#define FOREK_FORMULA_CORE_OPERATION_LTL_NEXT_HPP
...
template <typename T>
class Next : public Unary<T> {
   public:
    Next() = delete;
    explicit Next(std::unique_ptr<Node<T>> expr) : Unary<T>(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Next with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Next>(std::move(this->expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::ltl
#endif
