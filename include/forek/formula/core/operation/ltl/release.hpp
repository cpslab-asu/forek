#ifndef FOREK_FORMULA_CORE_OPERATION_PL_RELEASE_HPP
#define FOREK_FORMULA_CORE_OPERATION_PL_RELEASE_HPP
...
class Release : public Binary {
   public:
    Release() = delete;
    Release(std::unique_ptr<Node> lexpr, std::unique_ptr<Node> rexpr)
    ...
    auto accept(visitor::Visitor& visitor) const -> void override {
        try {
            ...
            throw visitor::VisitorException(
                "unable to visit Release with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const
        -> std::unique_ptr<core::Node> override {
        return std::make_unique<Release>(std::move(lexpr_->clone()),
