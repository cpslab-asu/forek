#ifndef FOREK_FORMULA_CORE_OPERATION_LTL_UNTIL_HPP
#define FOREK_FORMULA_CORE_OPERATION_LTL_UNTIL_HPP
...
template <typename T>
class Until : public Binary<T> {
   public:
    Until() = delete;
    Until(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Until with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Until>(std::move(this->lexpr_->clone()),
