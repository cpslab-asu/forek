#ifndef FOREK_FORMULA_CORE_OPERATION_STPL_UNIVERSE_HPP
#define FOREK_FORMULA_CORE_OPERATION_STPL_UNIVERSE_HPP
...
template <typename T>
class Universe : public Unary<T> {
   public:
    Universe() = delete;
    explicit Universe(std::unique_ptr<Node<T>> expr) : Unary<T>(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Universe with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Universe>(std::move(this->expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::stpl
