#ifndef FOREK_FORMULA_CORE_OPERATION_STPL_COMPLEMENT_HPP
#define FOREK_FORMULA_CORE_OPERATION_STPL_COMPLEMENT_HPP
...
template <typename T>
class Complement : public Unary<T> {
   public:
    Complement() = delete;
    explicit Complement(std::unique_ptr<Node<T>> expr) : Unary<T>(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Complement with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Complement>(std::move(this->expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::stpl
