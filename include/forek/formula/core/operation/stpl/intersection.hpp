#ifndef FOREK_FORMULA_CORE_OPERATION_STPL_INTERSECTION_HPP
#define FOREK_FORMULA_CORE_OPERATION_STPL_INTERSECTION_HPP
...
template <typename T>
class Intersection : public Binary<T> {
   public:
    Intersection() = delete;
    Intersection(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Intersection with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Intersection>(std::move(this->lexpr_->clone()),
