#ifndef FOREK_FORMULA_CORE_OPERATION_STPL_RELEASE_HPP
#define FOREK_FORMULA_CORE_OPERATION_STPL_RELEASE_HPP
...
template <typename T>
class ReleaseSpatial : public Binary<T> {
   public:
    ReleaseSpatial() = delete;
    ReleaseSpatial(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit ReleaseSpatial with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<ReleaseSpatial>(std::move(this->lexpr_->clone()),
                                                std::move(this->rexpr_->clone()));
    }
};

template <typename T>
class ReleaseSpatialBounded : public Binary<T> {
   public:
    const mtl::types::Interval interval_;

   public:
    ReleaseSpatialBounded() = delete;
    ReleaseSpatialBounded(std::unique_ptr<core::Node<T>> lexpr,
                          std::unique_ptr<core::Node<T>> rexpr, mtl::types::Interval interval)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException(
                "unable to visit ReleaseSpatialBounded with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<ReleaseSpatialBounded>(
