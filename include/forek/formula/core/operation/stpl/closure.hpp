#ifndef FOREK_FORMULA_CORE_OPERATION_STPL_CLOSURE_HPP
#define FOREK_FORMULA_CORE_OPERATION_STPL_CLOSURE_HPP
...
template <typename T>
class Closure : public Unary<T> {
   public:
    Closure() = delete;
    explicit Closure(std::unique_ptr<Node<T>> expr) : Unary<T>(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Closure with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Closure>(std::move(this->expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::stpl
