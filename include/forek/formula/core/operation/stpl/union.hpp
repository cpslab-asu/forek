#ifndef FOREK_FORMULA_CORE_OPERATION_STPL_UNION_HPP
#define FOREK_FORMULA_CORE_OPERATION_STPL_UNION_HPP
...
template <typename T>
class Union : public Binary<T> {
   public:
    Union() = delete;
    Union(std::unique_ptr<Node<T>> lexpr, std::unique_ptr<Node<T>> rexpr)
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit Union with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<Union>(std::move(this->lexpr_->clone()),
