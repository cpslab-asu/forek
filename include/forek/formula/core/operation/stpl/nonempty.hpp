#ifndef FOREK_FORMULA_CORE_OPERATION_STPL_NONEMPTY_HPP
#define FOREK_FORMULA_CORE_OPERATION_STPL_NONEMPTY_HPP
...
template <typename T>
class NonEmpty : public Unary<T> {
   public:
    NonEmpty() = delete;
    explicit NonEmpty(std::unique_ptr<Node<T>> expr) : Unary<T>(std::move(expr)) {}
    ...
    auto accept(visitor::Visitor<T>& visitor) -> void override {
        try {
            ...
            throw visitor::VisitorException("unable to visit NonEmpty with provided visitor");
        }
    }

    [[nodiscard]] inline auto clone() const -> std::unique_ptr<core::Node<T>> override {
        return std::make_unique<NonEmpty>(std::move(this->expr_->clone()));
    }
};
}  // namespace forek::formula::core::operation::stpl
